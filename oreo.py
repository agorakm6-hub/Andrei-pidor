...
async def render_screen(message: Message, state: FSMContext, text: str, inline_markup=None) -> None:
    ...
    # ОТПРАВЛЯЕМ REPLY-КЛАВИАТУРУ ОТДЕЛЬНО (она всегда видна под полем ввода)
    ...
    if screen_msg_id:
        try:
            ...
            # Отправляем Reply-клавиатуру, если её нет
            try:
                await bot.send_message(chat_id, "⬇️ КНОПКИ ВНИЗУ", reply_markup=reply_markup)
            except Exception:
                pass
            return
        except Exception as e:
...
# ============ ОБРАБОТЧИК КНОПКИ "ДОПОЛНИТЕЛЬНО" (REPLY) ============
