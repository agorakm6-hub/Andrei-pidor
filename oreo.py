...
- Все задания ТОЛЬКО в Alert-окнах (по центру экрана)
- В доказательстве для куратора пишется полный текст задания
- Кнопка "МОЙ ПУТЬ" показывает прогресс в Alert
...
def main_menu_keyboard(user_id: int) -> InlineKeyboardMarkup:
    ...
    if task_num == 0:
        ...
        return InlineKeyboardMarkup(inline_keyboard=[
            ...
            [InlineKeyboardButton(text="МОЙ ПУТЬ", callback_data="my_path", style="primary")],
...
async def render_screen(message: Message, state: FSMContext, text: str, reply_markup=None) -> None:
    ...
    if screen_msg_id:
        try:
            if has_avatar:
                ...
                await bot.edit_message_media(chat_id=chat_id, message_id=screen_msg_id, media=media, reply_markup=reply_markup)
            else:
                await bot.edit_message_text(text=monotext, chat_id=chat_id, message_id=screen_msg_id, reply_markup=reply_markup, parse_mode="HTML")
            return
        except Exception as e:
    ...
    try:
        if has_avatar:
            photo = FSInputFile(BOT_AVATAR)
            sent = await bot.send_photo(chat_id, photo=photo, caption=monotext, reply_markup=reply_markup, parse_mode="HTML")
        else:
            sent = await bot.send_message(chat_id, text=monotext, reply_markup=reply_markup, parse_mode="HTML")
        if sent:
...
# ============ МОЙ ПУТЬ (ТОЛЬКО ALERT) ============
@router.callback_query(F.data == "my_path")
async def my_path(callback: CallbackQuery) -> None:
    user_id = callback.from_user.id
    user = get_user(user_id)
    if user.get('banned', False):
        await callback.answer("🚫 ВЫ ЗАБАНЕНЫ!", show_alert=True)
    ...
    if task_num == 0:
        ...
        return
    await callback.answer(
        f"🐋 ВАШ ПУТЬ\n\nТЕКУЩЕЕ ЗАДАНИЕ: #{task_num}\nВЫПОЛНЕНО: {completed} из 50\nОСТАЛОСЬ: {remaining}\n\nПРОГРЕСС: {'█' * int((completed/50)*20)}{'░' * (20 - int((completed/50)*20))}",
        show_alert=True
    )
    await callback.answer()
